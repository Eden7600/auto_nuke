defmodule AutoNuke.Operator.CondenserFill.DrainValve do
  ...
  @drain_valve Valves.condenser_drain()
  ...
  def open(%DV{}, fill) do
    Logger.info(@log_prefix <> "Fill at #{ceil(fill)}% and rising, opening drain.")
  ...
  def hold(%DV{}, fill) do
    ...
    Logger.info(
      @log_prefix <> "Fill at #{ceil(fill)}% and dropping, holding drain at #{round(open)}%."
    )
  ...
  def close(%DV{open: true}, fill) do
    Logger.info(@log_prefix <> "Fill at #{ceil(fill)}%, closing drain.")
