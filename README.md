...
- [`mix auto_nuke.refuel`](lib/mix/tasks/refuel.ex) — Guided reactor refueling: safety checks, fuel report, drains the core pool, walks each bay through raising the piston and opening/closing the hatch, then refills the pool.  Pistons are left raised — lowering them is a startup step, not a refueling one.  (The crane has no API — the cell swaps are up to you.)
